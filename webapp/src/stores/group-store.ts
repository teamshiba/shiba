...
export class GroupStore {
  ...
  async updateActiveGroups(): Promise<void> {
    this.activeGroups = unwrap(
      (
        await axios.get<Message<Group[]>>(
          `${serverPrefix}/room/list?state=false`
        )
      ).data
    );
  }
  ...
  async updateHistoryGroups(): Promise<void> {
    this.historyGroups = unwrap(
      (
        await axios.get<Message<Group[]>>(
          `${serverPrefix}/room/list?state=true`
        )
      ).data
    );
  }
...
class GroupProfileStore {
  ...
  async update() {
    this.data = unwrap(
      (await axios.get<Message<Group>>(`${serverPrefix}/room/${this.groupId}`))
        .data
    );
  }
